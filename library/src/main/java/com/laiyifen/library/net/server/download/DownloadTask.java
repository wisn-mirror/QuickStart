...
import com.laiyifen.library.net.db.DownloadManager;
import com.laiyifen.library.net.exception.HttpException;
import com.laiyifen.library.net.exception.NetException;
import com.laiyifen.library.net.exception.StorageException;
import com.laiyifen.library.net.model.HttpHeaders;
import com.laiyifen.library.net.model.Progress;
import com.laiyifen.library.net.server.NetDownload;
import com.laiyifen.library.net.server.task.PriorityRunnable;
...
/**
 * Created by Wisn on 2018/5/3 下午1:59.
 */
...
public class DownloadTask implements Runnable {
    ...
    public DownloadTask folder(String folder) {
        if (folder != null && !TextUtils.isEmpty(folder.trim())) {
            ...
            NetLog.w("folder is null, ignored!");
        }
        return this;
    }
    ...
    public DownloadTask fileName(String fileName) {
        if (fileName != null && !TextUtils.isEmpty(fileName.trim())) {
            ...
            NetLog.w("fileName is null, ignored!");
        }
        return this;
    }
    ...
    public void start() {
        ...
        if (progress.status == Progress.NONE || progress.status == Progress.PAUSE || progress.status == Progress.ERROR) {
            ...
            NetLog.w("the task with tag " + progress.tag + " is already in the download queue, current task status is " + progress.status);
        }
    }
    ...
    public void pause() {
        ...
        if (progress.status == Progress.WAITING) {
            ...
            NetLog.w("only the task with status WAITING(1) or LOADING(2) can pause, current status is " + progress.status);
        }
    }
    ...
    public void run() {
        ...
        if (startPosition < 0) {
            postOnError(progress, NetException.BREAKPOINT_EXPIRED());
            return;
        }
        if (startPosition > 0) {
            if (!TextUtils.isEmpty(progress.filePath)) {
                File file = new File(progress.filePath);
                if (!file.exists()) {
                    postOnError(progress, NetException.BREAKPOINT_NOT_EXIST());
                    return;
                }
            }
        }
        ...
        if (progress.status == Progress.PAUSE) {
            ...
            postOnError(progress, NetException.UNKNOWN());
        }
    }
