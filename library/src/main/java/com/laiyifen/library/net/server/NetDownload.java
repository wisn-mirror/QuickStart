...
import com.laiyifen.library.net.server.download.DownloadTask;
import com.laiyifen.library.net.server.download.DownloadThreadPool;
...
/**
 * Created by Wisn on 2018/5/3 下午1:55.
 */
...
public class NetDownload {
    ...
    public void startAll() {
        for (Map.Entry<String, DownloadTask> entry : taskMap.entrySet()) {
            ...
            if (task == null) {
                NetLog.w("can't find task with tag = " + entry.getKey());
                continue;
            }
            task.start();
        }
