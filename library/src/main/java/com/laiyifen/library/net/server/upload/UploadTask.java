...
import com.laiyifen.library.net.base.ProgressRequestBody;
import com.laiyifen.library.net.base.Request;
import com.laiyifen.library.net.db.UploadManager;
import com.laiyifen.library.net.model.Progress;
import com.laiyifen.library.net.model.Response;
import com.laiyifen.library.net.server.NetUpload;
...
public class UploadTask<T> implements Runnable {
    ...
    public UploadTask(String tag, Request<T, ? extends Request> request) {
        ...
        executor = NetUpload.getInstance().getThreadPool().getExecutor();
    ...
    public UploadTask<T> start() {
        if (NetUpload.getInstance().getTask(progress.tag) == null || UploadManager.getInstance().get(progress.tag) == null) {
        ...
        if (progress.status != Progress.WAITING && progress.status != Progress.LOADING) {
            ...
            NetLog.w("the task with tag " + progress.tag + " is already in the upload queue, current task status is " + progress.status);
        }
        return this;
    }
    ...
    public UploadTask<T> remove() {
        ...
        UploadTask<T> task = (UploadTask<T>) NetUpload.getInstance().removeTask(progress.tag);
